...
def train_and_evaluate(model, train_loader, val_loader,test_loader, weights, optimizer, epochs, num_tasks, model_name, analysis,names,fold,metrics_per_subset,mean_confusion_matrix_bact,mean_confusion_matrix_fung,mean_confusion_matrix_ameb):
   ...
   # Go through the number of epochs
    ...
    for task_idx in range(num_tasks):
        ...
        if(task_idx==0):
            task_preds_test = np.where(task_probs_test >= thresholds[task_idx], 1, 0)
        if(task_idx==1):
            task_preds_test =np.where(task_probs_test>= thresholds[task_idx], 1, 0)
        if(task_idx==2):
            task_preds_test = np.where(task_probs_test>= thresholds[task_idx], 1, 0) 
...
if __name__ == "__main__":
    ...
    analysis='infections_multitask_v2_lossandroc_final_200'
    ...
    pickle_files = {
        "metrics_per_subset_lossandroc.pkl": metrics_per_subset,
        "confidence_intervals_lossandroc.pkl": confidence_intervals,
        "pvalues_lossandroc.pkl": p_values,
        "p_values_corrected_lossandroc.pkl":corrected_p_values_dict
    }
