...
class MultitaskDenseNet121(torch.nn.Module):
    ...
    def forward(self, inputs):
        ...
        # TODO: Changed this in the train_and_evaluate function
        ...
        # Step 2: dropout and other layers
...
def train_and_evaluate(model, train_loader, val_loader,test_loader, weights, optimizer, epochs, num_tasks, model_name, analysis,fold,metrics_per_subset,mean_confusion_matrix_bact,mean_confusion_matrix_fung,mean_confusion_matrix_ameb):
    ...
    for task_idx in range(num_tasks):
        ...
        if(task_idx==0):
                task_preds_test = np.where(all_probs_test[task_idx] >= thresholds[task_idx], 1, 0)
        if(task_idx==1):
                task_preds_test =np.where(all_probs_test[task_idx] >= thresholds[task_idx], 1, 0)
        if(task_idx==2):
                task_preds_test = np.where(all_probs_test[task_idx] >= thresholds[task_idx], 1, 0)   
...
if __name__ == "__main__":
    ...
    analysis='infections_multitask_v1_final_200_lossandroc'
    ...
    pickle_files = {
        "metrics_per_subsets_lossandroc.pkl": metrics_per_subset,
        "confidence_intervals_lossandroc.pkl": confidence_intervals,
        "pvalues_lossandroc.pkl": p_values,
        "p_values_lossandroc.pkl":corrected_p_values_dict
    }
